...
def get_bot_info():
    try:
        response = requests.get(f'https://discord.com/api/v10/users/{bot_id}', headers=headers, timeout=10)
        ...
        print(f"{Fore.RED}Failed to get bot info: {e}")
        return None
...
def get_guilds():
    try:
        ...
        print(f"{Fore.RED}Failed to get guilds: {e}")
        return None

def get_application_info():
    try:
        response = requests.get('https://discord.com/api/v10/oauth2/applications/@me', headers=headers, timeout=10)
        if response.status_code == 200:
            return response.json()
        else:
        ...
        print(f"{Fore.RED}Failed to get application info: {e}")
        return None
